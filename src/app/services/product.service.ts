...
export class ProductService {
  ...
  crearGaleria(formData: FormData): Observable<any> {
    return this.http.post(`${this.baseUrl}/api/imagenes_gallery/upload_gallery`, formData); // Asegúrate de que esta URL sea la correcta
  }
  ...
  listarProductosPorCategoria(id_categoria: number): Observable<any[]> {
    return this.http.get<any[]>(`api/imagenes/upload/${id_categoria}`);
  }

  listarProductosPorCategoriaS(id_categoriaS: number[]): Observable<any[]> {
    return this.http.get<any[]>(`api/imagenes/upload/${id_categoriaS}`);
  }
